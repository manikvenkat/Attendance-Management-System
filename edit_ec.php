...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['submit'])) {
	   $sql = "SELECT * FROM `subwise` WHERE `subject`='EC'";
...
$sqll = "UPDATE `subwise` SET 
	   `present`='$present',`absent`='$absent',`percent`='$percent' WHERE `subject`='EC' AND `rollno`='$rollno'";
...
<form method="POST" action="edit_ec.php">
<?php
include 'connect.php';
$sql = "SELECT * FROM `subwise` WHERE `subject`='EC'";
$result = $conn->query($sql);	
