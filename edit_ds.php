...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['submit'])) {
	   $sql = "SELECT * FROM `subwise` WHERE `subject`='DS'";
...
$sqll = "UPDATE `subwise` SET 
	   `present`='$present',`absent`='$absent',`percent`='$percent' WHERE `subject`='DS' AND `rollno`='$rollno'";
...
<form method="POST" action="edit_ds.php">
<?php
include 'connect.php';
$sql = "SELECT * FROM `subwise` WHERE `subject`='DS'";
$result = $conn->query($sql);	
